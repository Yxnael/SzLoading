...
local oi
local SAok=pcall(function()local M=LP:GetMouse()oi=hookmetamethod(game,"__index",newcclosure(function(s,k)if st.sa and(not checkcaller or not checkcaller())and rawequal(s,M)then -- FIX: tgtA -> tgt (silent aim funciona em qualquer jogador)
local t=tgt()if t then if k=="Hit"then return pred(t)end if k=="Target"then return t end if k=="UnitRay"or k=="Origin"then local o=C.CFrame.Position return Ray.new(o,(t.Position-o).Unit)end end end return oi(s,k)end))end)
local on
local NCok=pcall(function()on=hookmetamethod(game,"__namecall",newcclosure(function(s,...)local a={...}if st.sa and(not checkcaller or not checkcaller())and typeof(s)=="userdata"and(s:IsA("RemoteEvent")or s:IsA("RemoteFunction"))then local n=string.lower(s.Name)if n:find("hoot")or n:find("ire")or n:find("gun")or n:find("weapon")then -- FIX: tgtA -> tgt (silent aim nos remotes também)
local t=tgt()if t then for i,v in ipairs(a)do if typeof(v)=="CFrame"then a[i]=CFrame.new(t.Position)end if typeof(v)=="Vector3"then a[i]=t.Position end if typeof(v)=="Ray"then a[i]=Ray.new(C.CFrame.Position,(t.Position-C.CFrame.Position).Unit)end end end end end return on(s,unpack(a))end))end)
local keys={}
...
task.spawn(function()local t1,t2,t3,t4=0,0,0,0 local lastAt=0 local lastEquip=0 while true do local dt=task.wait()local c=LP.Character local hrp=HRP()local hum=c and c:FindFirstChildOfClass("Humanoid")if hum then if not st.fl then hum.WalkSpeed=st.sp and(st.speed or 32)or 16 end if st.jf then hum.JumpPower=st.jf end end
...
-- FIX: Fly refeito com CFrame em vez de BodyVelocity+PlatformStand (não trava mais)
...
-- Aim
Mk(Pgs[3],"Silent Aim","sa",function(_,v)if v and not SAok and not NCok then N("SzDunamis","Sem hooks: silent aim inativo")end end)Mk(Pgs[3],"Auto Shoot","at")Mk(Pgs[3],"Kill Aura","kill")
-- Farm
