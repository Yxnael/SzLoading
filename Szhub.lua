-- ===== SP HUB | MM2 v2.0 (REESCRITO, SEM HOOKS, UNIVERSAL) =====
...
-- ===== GUARD (para re-executar sem travar) =====
...
-- ===== HELPERS DE ERRO (mostram tudo na tela) =====
...
local Tr = {
    en = { Skip="Skip", Box="Don't show this again",
    ...
    pt = { Skip="Pular", Box="Nao mostrar novamente",
...
local function L(k, lang)
    local t = Tr[lang] or Tr.en
    return t[k] or Tr.en[k] or k
end
local DEFAULT_LANG = "en"

-- ===== PERSISTÊNCIA (se o executor tiver readfile/writefile) =====
...
-- ===== MOUSE / SILENT AIM (sem hooks, via camera lock) =====
...
UserInputService.InputBegan:Connect(function(i)
    if FLYKEYS[i.KeyCode.Name] then keys[i.KeyCode.Name] = true end
end)
UserInputService.InputEnded:Connect(function(i)
    if FLYKEYS[i.KeyCode.Name] then keys[i.KeyCode.Name] = nil end
end)
...
local function unload()
    ...
    Notif("SP Hub", "Unloaded")
end

-- ===== MAIN LOOP (fecha automático quando ACTIVE=false) =====
RunService.Heartbeat:Connect(function(dt)
    ...
    pcall(function()
        ...
        -- Speed
        if hum and st.sp and hum.WalkSpeed ~= 32 then hum.WalkSpeed = 32 end
        if hum and not st.sp and hum.WalkSpeed ~= 16 then hum.WalkSpeed = 16 end
        ...
        -- Auto Shoot (bala se o alvo tem faca)
        if st.at then
...
-- Silent Aim: lock de câmera no alvo com faca
...
-- ===== UNLOAD loop (para o loop se ACTIVE ficar false) =====
task.spawn(function()
    while true do
        task.wait(1)
        if not ACTIVE then
            -- interrompe o while original da melhor forma (os loops usam ACTIVE)
            break
        end
    end
end)
...
local LoadRatio = Instance.new("UIAspectRatioConstraint", LoadingFrame); LoadRatio.AspectRatio = 1.3
...
-- Menu principal (seletor de idioma)
...
GameLabel.Text = IS_MM2 and "Murder Mystery 2 v2.0" or ("Unsupported | " .. tostring(game.PlaceId))
...
if IS_MM2 then
    ...
    Play.MouseEnter:Connect(function()
        TweenService:Create(Play, TweenInfo.new(.2), { BackgroundColor3 = Color3.fromRGB(180, 30, 255) }):Play()
    end)
    Play.MouseLeave:Connect(function()
        TweenService:Create(Play, TweenInfo.new(.2), { BackgroundColor3 = PUR }):Play()
    end)
    ...
    local function openCheats()
        -- Destroi loading e abre GUI cheats
        pcall(function() LoadingGui:Destroy() end)
        if getgenv then getgenv().SZHUB_GUI = buildCheatGUI() end
    end
    ...
    Notif("SP Hub", "Rodar apenas no MM2")
end
...
local function buildCheatGUI()
    ...
    local T = function(k) return L(k, DEFAULT_LANG) end
    ...
    Notif("SP Hub", "Carregado | RightShift = menu")
    return G
end
